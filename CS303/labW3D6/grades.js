...
quiz.scoreStudent = function (sid) {
    ...
    let score =this.students.find(score=>score.sid===sid);
    score.sort(answerComparator);
    let total=0;
    let count=0;
    let ansArray=[];
    for(let ans of score.answers){
        ansArray.push(ans);

    }
    for(let studentans of quiz)
           if(ansArray==studentans){
               count++;
               total+=studentans;
